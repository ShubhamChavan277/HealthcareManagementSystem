...
from book_doctor.models import Doctor, VidDoc, VidApp,  VAppointment, PhyDoc, PhyApp, PAppointment
# Create your views here.
def my_meeting(request):
    user = log.objects.get(email = request.session['email'])
    name = user.username
    vaps = VAppointment.objects.filter(log__username = name).order_by('date')
    return render(request, 'my_meeting.html', {'vaps' : vaps})

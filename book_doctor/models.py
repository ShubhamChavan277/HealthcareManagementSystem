...
class PhyApp(models.Model):
    phydoc = models.ForeignKey(PhyDoc, on_delete=models.CASCADE)
    tm = [
        (0, '09:00 - 09:30'),
        (1, '10:00 - 10:30'),
        (2, '11:00 - 11:30'),
    ...
    def __str__(self):
        return str(self.phydoc.doctor.name) + '-' + str(self.time)

class PAppointment(models.Model):
    log = models.ForeignKey(log, on_delete=models.CASCADE)
    doctor = models.ForeignKey(Doctor, on_delete=models.CASCADE)
    tslot = models.CharField(max_length=30)
    date = models.DateField()
